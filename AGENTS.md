...
- `devtools.js` starts capture on the first panel selection. `store.js` owns in-memory lifecycle; HTTP-only is the default. Only `probe.js` may execute inside the inspected page.
